...
def run_session(win=None, eeg=None, rounds=40, percent_green=75):
    ...
    def show_stimulus(win, color):
        text_cross = visual.TextStim(win=win, text='\u00D7', color=custom_color['grey'],
        ...
        if eeg:
            eeg.outlet.push_sample(x=[mark[color]], timestamp=time.time())
        circle.draw()
        win.flip()
